...
// _____TITLE:
const titleArr = title.replace(/\s+/g, ' ').trim().split(' ')
    for (let word of titleArr) {
        if (word.length > 12) errors.title = "Each word can't be larger than 12 characters"
...
if (title.length > 40) errors.title = "The title can't exceed 40 characters"
...
//______DESCRIPTION:
const summaryArr = summary.replace(/\s+/g, ' ').trim().split(' ')
    for (let word of summaryArr) {
        if (word.length > 12) errors.summary = "Each word can't be larger than 12 characters"
        if (word.length < 2) errors.summary = "Every word must contain at least 2 characters..."
    }
...
if (stepByStep.length > 0) {
    for (let step of stepByStep) {
        for (let word of step.split(' ')) {
            if (word && word.length > 12) errors.stepByStep = "Each word can't be larger than 12 characters"
            if (word && word.length < 2) errors.stepByStep = "Every word must contain at least 2 characters..."
        }
    }
}
